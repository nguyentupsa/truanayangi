...
import { personalFoods, personalSelector, excludedDishes } from '@/lib/personal-pool';
...
function FoodImage({food,language}:{food:Food;language:Language}){
 if(food.img)return <div role="img" aria-label={foodName(food,language)} className="food-image dish-photo" style={{backgroundImage:`url(${basePath}/${food.img})`,backgroundSize:'contain',backgroundPosition:'center',backgroundRepeat:'no-repeat'}}/>;
...
export default function Home(){
 ...
 const [budget,setBudget]=useState('50'),[custom,setCustom]=useState('50'),[diet,setDiet]=useState<'all'|'veg'|'nonveg'>('all'),[sound,setSound]=useState(true),[spinning,setSpinning]=useState(false),[result,setResult]=useState<Food|null>(null),[revealed,setRevealed]=useState(false);
 const [reel,setReel]=useState(()=>foods.filter(f=>!excludedDishes.has(f.image)).slice(0,12).map((food,id)=>({food,id}))),[moving,setMoving]=useState(false);
 ...
 useEffect(()=>{const saved=readCookie<Record<string,unknown>>('settings');if(saved&&typeof saved==='object'){if(typeof saved.budget==='string'&&['35','50','75','100','150','custom'].includes(saved.budget))setBudget(saved.budget);if(typeof saved.custom==='string'&&Number(saved.custom)>=30&&Number(saved.custom)<=180)setCustom(saved.custom);if(saved.diet==='all'||saved.diet==='veg'||saved.diet==='nonveg')setDiet(saved.diet);else if(typeof saved.veg==='boolean')setDiet(saved.veg?'veg':'all');if(typeof saved.sound==='boolean')setSound(saved.sound)}setPreferencesReady(true)},[]);
 useEffect(()=>{if(preferencesReady){try{writeCookie('settings',{budget,custom,diet,sound});setCookieError('')}catch{setCookieError(language==='vi'?'Không thể lưu cookie. Lựa chọn chỉ giữ trong lần mở trang này.':'Cookies unavailable. Preferences last only for this visit.')}}},[preferencesReady,budget,custom,diet,sound,language]);
 ...
 const eligible=useMemo(()=>population.filter(f=>diet==='all'?true:diet==='veg'?!!f.veg:!f.veg),[population,diet]);
 ...
 {!eligible.length&&<p className="preferences-message">{language==='vi'?'Pool không có món phù hợp. Đổi bộ lọc chay/mặn hoặc thêm món.':'No matching dishes. Change the diet filter or add dishes.'}</p>}
